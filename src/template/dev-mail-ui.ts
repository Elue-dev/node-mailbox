export default function DevMailUI({ path }: { path: string }): string {
  return `
  <!DOCTYPE html>
  ...
  <body class="animate-slideUp">
      <div class="header">
          ...
          <div class="header-actions">
              ...
              <button class="theme-toggle" onclick="refreshInbox()" title="Toggle theme">
              Refresh Inbox
              </button>
      ...
      <script>
          class EmailDatabase {
              ...
              async syncWithServer(serverEmails) {
                  ...
                  if (serverEmails.length > 0) {
                      const emailsToDelete = localEmails.filter(e => !serverEmailIds.has(e.id));
          ...
          async function clearAllEmails() {
              ...
              showConfirmationModal(
                  ...
                  async function() {
                      try {
                          // Try to clear from server if online
                          if (isOnline) {
                              try {
